...
def init_db():
    with get_db() as conn:
        ...
        for col, coltype in [('age', 'INTEGER'), ('phone', 'TEXT'), ('address', 'TEXT'), ('photo', 'TEXT')]:
            c.execute(f'''
                DO $$ BEGIN
...
@app.route('/personnel/<int:pid>/photo', methods=['POST'])
@login_required
@csrf_required
...
def dashboard_insights():
    ...
    with get_db() as conn:
        ...
        for row in c.fetchall():
            ...
            hr_conds = [co for co in conds if co in HIGH_RISK_CONDITIONS]
            if hr_conds:
                hr_ids.add(row['id'])
        ...
        if hr_ids:
            ...
            c.execute(
                ...
                WHERE personnel_id IN ({placeholders})
                  AND visit_date >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '5 months')
